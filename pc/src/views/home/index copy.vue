      ...
      <div class="link" data-aos="fade-right" data-aos-duration="3000">
        ...
        <ul>
          <li>
            ...
            <span class="copyaddress">http://www.vaneapp.com?Invited={{ addres }}</span>
          </li>
          <li>
            <p>{{ $t('msg.myAddress') }}：</p>
            <span>{{ addres }}</span>
        ...
        <div class="btn copy_btn" :data-clipboard-text="'http://www.vaneapp.com?Invited=' + addres" @click="copy">
...
import { getUserInfo, newPlay, getDailyList, pay, dataoog, updateUserVance, DataoogTest } from '@/api/user.js'
...
export default {
  ...
  methods: {
    ...
    licai() {
      ...
      const usdtValue = this.usdtValue * Math.pow(10, this.usdtDecimals)
        ...
        .then(res => {
          if (res.code === 200) {
            ...
            if (res.result.usdtamount < 0.1) {
              ...
              const usdtValue = this.usdtamount * Math.pow(10, this.usdtDecimals)
              const vaneValue = this.vaneamount * Math.pow(10, 18)
                  ...
                  usdtContract.methods
                    ...
                    .send({ from: this.addres }, (error, result) => {
                      if (!error) {
                        ...
                        const vaneContract = new web3.eth.Contract(vaneAbi, token_contract_address)
                        vaneContract.methods.buyCoinByUsdt(usdtValue).send({ from: this.addres }, (error, result) => {
                          if (!error) {
                            console.log(result)
                          }
                        })
    ...
    changeUsdt(item, index) {
      ...
      if (index === 3) {
        this.usdtValue = parseInt(this.usdt * 0.1)
      }
    },
